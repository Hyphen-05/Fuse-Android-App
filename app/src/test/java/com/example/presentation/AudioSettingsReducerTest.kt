...
class AudioSettingsReducerTest {
    ...
    fun setVisualizerPreset_ebbAndFlow_neverSnapsAndNudgesInstead() {
        ...
        assertEquals(55f, audio.hueBeatNudgeDeg)
        assertEquals(450f, audio.hueNudgeReturnMs)
        // F5 round 2: the brightness envelope is deliberately boxed into a narrow band so the
        // preset can't breathe. If these three drift back down, "flashy" comes back.
        assertEquals(0.62f, audio.visualizerMinBrightness)
        assertEquals(0.80f, audio.ambientCapFraction)
        ...
        assertTrue(effects.contains(AudioSideEffect.SaveAudioPrefFloat("hue_beat_nudge_deg", 55f)))
    ...
    // ========================================================================
    // SetFlashTimingOffsetMs — P1 predictive flash scheduling's by-ear slider,
    // same standalone-value pattern as SetBluetoothDelayMs above (no preset reset,
    // single pref write).
    // ========================================================================

    @Test
    fun setFlashTimingOffsetMs_updatesStateAndPersists() {
        val (newState, effects) = reduce(intent = RgbIntent.SetFlashTimingOffsetMs(120))

        assertEquals(120, newState.audioSettings.flashTimingOffsetMs)
        assertEquals(listOf(AudioSideEffect.SaveAudioPrefInt("flash_timing_offset_ms", 120)), effects)
    }
