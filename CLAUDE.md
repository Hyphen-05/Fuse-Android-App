...
5. **Phase 4 — audio capture hardware isolation IN PROGRESS (branch `phase4-hardware-isolation`, based on `wire-phase3b-reducers`, PR open against `wire-phase3b-reducers`).** BLE write/connection I/O and the MediaProjection screen-capture path used by Ambiance mode were judged out of scope for this slice and deliberately not touched — see below.
...
- Phase 4 branch `phase4-hardware-isolation` is based on `wire-phase3b-reducers`, **not** `refactor-checkpoint` — confirmed at the start of Phase 4 that `refactor-checkpoint` still predates RgbUiState/dispatch() entirely (no `RgbUiState.kt`, not an ancestor), so it was the wrong base despite an initial handoff note suggesting it. Its PR targets `wire-phase3b-reducers`. Re-verify with `git merge-base --is-ancestor` before assuming this is still true or that either branch has since merged.
