...
5. **Phase 4 — audio capture hardware isolation COMPLETE.** Branch `phase4-hardware-isolation`, based on `wire-phase3b-reducers`; PR #12 merged into `wire-phase3b-reducers` 2026-07-20. `wire-phase3b-reducers` (including Phase 4) was then merged into `refactor-checkpoint` via PR #13 (2026-07-20, merge commit `d2024cc`) — `refactor-checkpoint` now contains all of Phases 1-4. **Not yet promoted to `main`**, which is still 26 commits behind `refactor-checkpoint` — that promotion is gated on a manual on-device smoke test of music sync (`phone_mic` and `on_device` modes) that is still outstanding. BLE write/connection I/O and the MediaProjection screen-capture path used by Ambiance mode were judged out of scope for this slice and deliberately not touched — see below.
   ...
   **Still outstanding, same caveat as Phase 3b:** no real on-device mic/Visualizer hardware behavior is exercised by unit tests (Robolectric shadows don't model real audio input) — only construction/contract (`AudioCaptureSourceTest.kt`) and the pure DSP math (`AudioDspProcessorTest.kt`) are unit-tested. A manual on-device smoke test of music sync (both `phone_mic` and `on_device` modes) is still outstanding before trusting this in production.
...
All four reducers below are **wired live into `RgbControllerViewModel`** (originally on branch `wire-phase3b-reducers`, based on `phase3b-complete`; as of PR #13 (2026-07-20) this is merged into `refactor-checkpoint`, which is now the up-to-date branch — not yet merged into `main`, pending an on-device smoke test). `RgbControllerViewModel._uiState` is a real `MutableStateFlow<RgbUiState>` (migrated in Stage 2/3, commits `24b60f3`/`8d39aac`), not the old flat `ControllerUiState`.
...
- `AudioCaptureSourceTest.kt` (`hardware/audio/`) and `AudioDspProcessorTest.kt` (`core/audio/`) cover the Phase 4 audio isolation — 10 tests total (construction/contract for the capture sources + fake, deterministic DSP-math assertions via synthetic frames). Full suite is **150 tests** as of Phase 4. Same caveat as above: real on-device mic/Visualizer hardware behavior is unverified by these tests (Robolectric doesn't model real audio input) — on-device smoke test of `phone_mic` and `on_device` music sync modes is still outstanding.

## Git/GitHub notes

- **`refactor-checkpoint` is now the up-to-date branch** (as of PR #13, merge commit `d2024cc`, 2026-07-20) — it contains all of Phases 1-4: the RgbUiState migration, all four reducers wired via `dispatch(RgbIntent)`, and Phase 4 audio hardware isolation. `wire-phase3b-reducers` and `phase4-hardware-isolation` are both fully merged into it and were left undeleted. Re-verify with `git merge-base --is-ancestor` before assuming this is still true, since branch pointers here can go stale.
- **`main` is not yet updated** — it is 26 commits behind `refactor-checkpoint` (still on the pre-RgbUiState flat `ControllerUiState` architecture). Promoting `refactor-checkpoint` → `main` is intentionally gated on a manual on-device smoke test of music sync (`phone_mic` and `on_device` modes) that is still outstanding — do not merge into `main` until that test has run.
